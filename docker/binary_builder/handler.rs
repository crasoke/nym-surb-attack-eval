...
impl Handler {
    ...
    async fn handle_surb_request(
        &mut self,
        recipient_tag: AnonymousSenderTag,
        amount: u32,
        connection_id: Option<u64>,
    ) -> Option<ServerResponse> {
        info!("Attempting request {amount} SURBs.");
    ...
    async fn handle_request(&mut self, request: ClientRequest) -> Option<ServerResponse> {
        match request {
            ...
            ClientRequest::AdditionalSurbsRequest { 
                recipient,
                amount,
                connection_id,
            } => self.handle_surb_request(recipient, amount, connection_id).await,
        }
    }
